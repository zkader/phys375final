...
class MS_Star:
    ...
    def CoupledODEs(self,R,init): #odes vectorized
        ...
        dT_dr = -1*np.minimum(3.*kappa*Rho*L/(16.*pi*c*self.a*T*T*T*R*R),(1.0-1.0/self.gamma)*(T/P)*Grav)
        dM_dr = SA*Rho
        dL_dr = SA*Rho*eps
...
#        print scale
    ...
    def ODEsolve(self,R,Rho,T):
        ...
        dydr.set_integrator('dop853')#,rtol=1e-6, nsteps=1000, first_step=1e-8,beta=0.08,verbosity=True)
